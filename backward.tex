...
\begin{tikzpicture}[auto, >=triangle 45]
  ...
  node [block, below of=suma1] (BH) {$[B.H]$}
  node [sum, below of=BH] (suma2) {\suma}
  node [block, below of=suma2] (B) {$B$}
  node [sum, left of=AH] (suma3) {\suma}
  node [sum, left of=CH] (suma4) {\suma}
  ;
  ...
  \draw [->] ($(DWTB) + (0.5cm,-1cm)$) -- node {iWDT} (BHp);
