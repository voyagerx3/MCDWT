...
\begin{tikzpicture}[auto, >=triangle 45]
  ...
  \draw node [node distance=3.5cm, right of=AL] (DWTA) {\DWT{A.L}{A.H}};
  \draw node [block, right of=DWTA] (BL) {$[B.L]$};
  \draw node [node distance=3.5cm, right of=BL] (DWTB) {\DWT{B.L}{\tilde{B}.H}};
  \draw node [block, right of=DWTB] (CL) {$[C.L]$};
  \draw node [node distance=3.5cm, right of=CL] (DWTC) {\DWT{C.L}{C.H}};
