...
\begin{tikzpicture}[auto, >=triangle 45]
  ...
  \draw
  node [block] (AL) {$[A.L]$}
  node [node distance=3.5cm, right of=AL, label=$A$] (DWTA) {\DWT{A.L}{A.H}}
  node [block, right of=DWTA] (BL) {$[B.L]$}
  node [node distance=3.5cm, right of=BL, label=$B$] (DWTB) {\DWT{B.L}{B.H}}
  node [block, right of=DWTB] (CL) {$[C.L]$}
  node [node distance=3.5cm, right of=CL, label=$C$] (DWTC) {\DWT{C.L}{C.H}}
