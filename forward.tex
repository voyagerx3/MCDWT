...
\tikzset{%
  block/.style    = {draw, thick, rectangle, minimum height = 2cm,
...
\begin{tikzpicture}[auto, thick, node distance=2cm, >=triangle 45]
  ...
  node [block, below of=AH] (BHA) {$[B.H_A]$}
  node [block, below of=CH] (BHC) {$[B.H_C]$};
  ...
  \draw % Arrows
      ...
      [->] (DWTB) -- node {iWDT} (BH)
      [->] (DWTC) -- node {iWDT} (CH)
      [->] (AH) -- node {$[B.L]\rightarrow [A.L]$} (BHA)
      [->] (CH) -- node {$[B.L]\rightarrow [C.L]$} (BHC);
